...
import org.gradle.test.fixtures.gradle.*
...
abstract class AbstractMavenModule extends AbstractModule implements MavenModule {
    ...
    void assertChecksumsPublishedFor(File File) {
        def sha1File = sha1File(File)
        sha1File.assertIsFile()
        assert HashCode.fromString(sha1File.text) == Hashing.sha1().hashFile(File)
        if (extraChecksums && !(File.name in missingExtraChecksums)) {
            def sha256File = sha256File(File)
            sha256File.assertIsFile()
            assert HashCode.fromString(sha256File.text) == Hashing.sha256().hashFile(File)
            def sha512File = sha512File(File)
            sha512File.assertIsFile()
            assert HashCode.fromString(sha512File.text) == Hashing.sha512().hashFile(File)
        }
        def md5File = md5File(File)
        md5File.assertIsFile()
        assert HashCode.fromString(md5File.text) == Hashing.md5().hashFile(File)
    }
    ...
    TestFile publishArtifact(Map<String, ?> artifact) {
